...
> [!NOTE]
> По умолчанию приложение запускается на `127.0.0.1:8080`. Если нужен другой порт, укажите `HTTP_PORT` при запуске, например: `make up HTTP_PORT=18080`.
>
> Чтобы не передавать порт каждой Make-команде в текущей shell-сессии, можно заранее выполнить `export HTTP_PORT=18080`. После этого обычные `make up`, `make smoke` и другие команды будут использовать указанное значение.
